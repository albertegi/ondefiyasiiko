...
public class EmailDomainValidator implements ConstraintValidator<NonDisposableEmail, String> {
    ...
    public EmailDomainValidator(
            @Value("${spring.app.security.disposable-email}")
            final List<String> domains) {
        this.blocked = domains.stream()
                .map(String::toLowerCase)
                .collect(Collectors.toSet());
//        this.blocked = domains.stream().map(d->d.toLowerCase()).collect(Collectors.toSet());
    }
    ...
    public boolean isValid(String email, ConstraintValidatorContext context) {
        ...
        final int atIndex = email.indexOf("@") + 1; // albert@gmail.com
